...
public class ZombieManager : MonoBehaviour
{
    ...
    public Transform[] spawnPonitList;
    ...
    private void SpawnARandomZombie()
    {
        ...
        {
            int index = Random.Range(0, spawnPonitList.Length);
            GameObject go = GameObject.Instantiate(zombiePrefab, spawnPonitList[index].position, Quaternion.identity);

            zombieList.Add(go.GetComponent<Zombie>());
            go.GetComponent<SpriteRenderer>().sortingOrder = spawnPonitList[index].GetComponent<SpriteRenderer>().sortingOrder;
        }
    }
